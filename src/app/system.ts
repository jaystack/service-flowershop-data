...
export default new System()
  ...
  .add('rabbitConn', Amqp.Connection()).ignorable().dependsOn({ component: 'config', source: 'rabbit', as: 'config' }, 'endpoints')
  .add('rabbitChannel', Amqp.Channel()).ignorable().dependsOn({ component: 'rabbitConn', as: 'connection' })
  .add('rabbitLogger', RabbitLogger()).ignorable().dependsOn('config', 'rabbitChannel', 'logger')
  .add('app', App())
